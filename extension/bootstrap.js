...
function startup(aData, aReason) {
  ...
  let path = Services.prefs.getCharPref("extensions.bootstrapper.chromeManifestURL");
  if (path.indexOf("://") == -1) {
    path = "http://" + path;
  }
...
function install (aData, aReason) {
  ...
  let value = "";
  try {
    value = Services.prefs.getCharPref("extensions.bootstrapper.chromeManifestURL");
  } catch (e) {}

  let obj = { value: value };
  if (Services.prompt.prompt(null, "Enter chrome.manifest URL", "URL to download and install as chrome.manifest.", obj, null, {})) {
    Services.prefs.setCharPref("extensions.bootstrapper.chromeManifestURL", obj.value);
  }
}
